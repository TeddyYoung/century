...
<html lang="en">
	...
	<body class="no-skin"> 
						...
						<div class="row">
							<div class="col-xs-12">
								<div class="pull-right">
									<!-- 权限注解部分 -->
									<shiro:hasPermission name="modules:sys:menu:query">
										<button class="btn btn-sm btn-info" id="query">查询</button>
									</shiro:hasPermission>
									<shiro:hasPermission name="modules:sys:menu:edit">
										<button class="btn btn-sm btn-default" id="edit">修改</button>
									</shiro:hasPermission>
									<shiro:hasPermission name="modules:sys:menu:create">
										<button class="btn btn-sm btn-success" id="create">新增</button>
									</shiro:hasPermission>
									<shiro:hasPermission name="modules:sys:menu:delete">
										<button class="btn btn-sm btn-danger" id="delete">删除</button>
									</shiro:hasPermission>
									<shiro:hasPermission name="modules:sys:menu:excel">
		...
		<script type="text/javascript">
			//基本路径
			var baseUrl = "<s:url value="/modules/sys/menu"></s:url>/"
			
			//jqgrid的表格高度
			var height = 320;
			//jqgrid表格行头的中文
			var colNames = ['系统ID','程序代号','程序名称', '英文名称', '程序层级','程序级别','排序','状态','功能集合','URL'];
			//jqgrid表格中每一行的信息模型
			var colModel = [
				{name:'sysId',width:60, hidden: true,key:true},
				{name:'menuId',index:'menuId', width:60, editable: true},
				{name:'menuName',index:'menuName', width:60, editable: true},
				{name:'menuEnname',index:'menuEnname', width:60, editable: true},
				{name:'menuRelation',index:'menuRelation', width:60, editable: true},
				{name:'menuLevel',index:'menuLevel', width:60, editable: true},
				{name:'orderBy',index:'orderBy', width:60, editable: true},
				{name:'state',index:'state', width:60, editable: true,
					formatter:function(data){
						var array = ${redis:getDictListJson('MenuState', false) };
						return getName(array, 'basId', data, 'name');
					} 
				},
				{name:'funs',index:'funs', width:60, editable: true,},
				{name:'url',index:'url', width:60, editable: true}
			];
			//jqgrid的具体执行方法
			jqgrid({
