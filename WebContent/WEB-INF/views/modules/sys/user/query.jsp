...
<html lang="en">
	...
	<body class="no-skin">
						...
						<div class="row">
							<div class="col-xs-12">
								<div class="pull-right" style="margin-right: 20px;margin-bottom: 10px;">
									<!-- 权限按钮拦截部分 -->
									<shiro:hasPermission name="modules:sys:user:query">
										<button class="btn btn-info" id="query">查询</button>
									</shiro:hasPermission>
								<%-- 	<shiro:hasPermission name="modules:sys:user:edit">
										<button class="btn btn-sm btn-default" id="edit">修改</button>
									</shiro:hasPermission>
									<shiro:hasPermission name="modules:sys:user:create">
										<button class="btn btn-sm btn-success" id="create">新增</button>
									</shiro:hasPermission>
									<shiro:hasPermission name="modules:sys:user:delete">
										<button class="btn btn-sm btn-danger" id="delete">删除</button>
									</shiro:hasPermission>
									<shiro:hasPermission name="modules:sys:user:excel">
		...
		<script type="text/javascript">
			//基本路径
			var baseUrl = "<s:url value="/modules/sys/user"></s:url>/"
			
			//jqgrid的表格高度
			var height = 320;
			//jqgrid表格行头的中文
			var colNames = ['','手机号码','用户昵称','性别'];
			//jqgrid表格中每一行的信息模型
			var colModel = [
				{name:'sysUser.sysId', width:60, hidden: true,key:true},
				{name:'sysLogin.mobile', width:60, editable: true},
				{name:'sysUser.nickName', width:60, editable: true},
				{name:'sysUser.gender', width:60, editable: true,
			...
			function create() {
				...
				$.ajax({
				   type: "post",
				   url: "<s:url value="/api/sys/user/isExistForUserId"/>",
				   data: {userId : userId},
				   async: false,
				   success: function(data){
					   if(data) {
							showGritter('消息提示', '您当前所填写的用户ID已存在');//消息提示
							flag = false;
						}
				   }
				});
				if(flag) {
					$.ajax({
					   type: "post",
					   url: "<s:url value="/api/sys/user/isExistForEmail"/>",
					   data: {email : email},
					   async: false,
					   success: function(data){
						   if(data) {
							   showGritter('消息提示', '您当前所填写的邮箱已被其他帐号绑定');//消息提示
								flag = false;
							}
					   }
					});
				...
				if(flag) {
					$.ajax({
					   type: "post",
					   url: "<s:url value="/api/sys/user/isExistForMobile"/>",
					   data: {mobile : mobile},
					   async: false,
					   success: function(data){
						   if(data) {
							   showGritter('消息提示', '您当前所填写的电话号码已被其他帐号绑定');//消息提示
								flag = false;
							}
					   }
					});
