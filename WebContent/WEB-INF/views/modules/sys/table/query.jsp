...
<html lang="en">
	...
	<body class="no-skin">
					...
					<div class="page-content">
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<shiro:hasPermission name="modules:sys:table:query">
									<button class="btn btn-sm btn-info" id="query">查询</button>
								</shiro:hasPermission>
								<shiro:hasPermission name="modules:sys:table:edit">
									<button class="btn btn-sm btn-default" id="edit">修改</button>
								</shiro:hasPermission>
								<shiro:hasPermission name="modules:sys:table:create">
									<button class="btn btn-sm btn-success" id="create">新增</button>
								</shiro:hasPermission>
								<shiro:hasPermission name="modules:sys:table:delete">
									<button class="btn btn-sm btn-danger" id="delete">删除</button>
								</shiro:hasPermission>
								<shiro:hasPermission name="modules:sys:table:excel">
		...
		<script type="text/javascript">
			//基本路径
			var baseUrl = "<s:url value="/modules/sys/table"></s:url>/"
			
			//jqgrid的表格高度
			var height = 320;
			//jqgrid表格行头的中文
			var colNames = ['','表逻辑名称','表物理名称', '最大的SYSID', '前缀位数','流水号位数','流水号最小值','流水号最大值'];
			//jqgrid表格中每一行的信息模型
			var colModel = [
				{name:'sysId', width:60, hidden: true,key:true},
				{name:'name',index:'name', width:60, editable: true},
				{name:'tableName',index:'tableName', width:60, editable: true},
				{name:'maxSysId',index:'maxSysId', width:60, editable: true},
				{name:'preNum',index:'preNum', width:60, editable: true},
				{name:'serialNum',index:'serialNum', width:60, editable: true},
				{name:'minVal',index:'minVal', width:60, editable: true},
				{name:'maxVal',index:'maxVal', width:60, editable: true}
			];
