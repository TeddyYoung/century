...
<html lang="en">
	...
	<body class="no-skin">
						...
						<div class="row">
							<div class="col-xs-12">
								<div class="pull-right">	
									<shiro:hasPermission name="modules:century:banner:query">
										<button class="btn btn-sm btn-info" id="query">查询</button>
									</shiro:hasPermission>
									<shiro:hasPermission name="modules:century:banner:edit">
										<button class="btn btn-sm btn-default" id="edit">修改</button>
									</shiro:hasPermission>
									<shiro:hasPermission name="modules:century:banner:create">
										<button class="btn btn-sm btn-success" id="create">新增</button>
									</shiro:hasPermission>
									<shiro:hasPermission name="modules:century:banner:delete">
		...
		<script type="text/javascript">
			//基本路径
			var baseUrl = "<s:url value="/modules/century/social"></s:url>/"
			
			//jqgrid的表格高度
			var height = 320;
			//jqgrid表格行头的中文
			var colNames = ['','头像','昵称','内容','操作'];
			//jqgrid表格中每一行的信息模型
			var colModel = [
				{name:'sysId', width:60, hidden: true,key:true},
				{name:'sendUserImage',index:'sendUserImage', width:60, editable: true, 
					formatter : function(value, grid, rows, state) {
								var q = "<img src='${pageContext.request.contextPath}/api/sys/picture/downloadSmallPic/"+ value +"'/>";
						return q;
					}
				},
				{name:'sendUserNickName',index:'sendUserNickName', width:60, editable: true},
				{name:'content',index:'content', width:60, editable: true},
				{name:'content',index:'sysId', width:60, editable: true,
					formatter : function(value, grid, rows, state) {
						var q = "["+"<a href='#'  style='color:#f60' onclick=feedback('" + rows.sendUserId + "')>查看</a>"+"]"
							    +"["+"<a href='#'  style='color:#f60' onclick=feedback('" + rows.sendUserId + "')>回复</a>"+"]";
						return q;
					}	
				}
			];
			...
			//----------------------------------------------------------
			function feedback(sendUserId) {
				location.href='<s:url value="/modules/century/social/chat/"/>' + sendUserId;
			}
		</script>
