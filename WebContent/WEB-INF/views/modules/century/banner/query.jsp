...
<html lang="en">
	...
	<body class="no-skin">
					...
					<div class="page-content">
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<%-- 	<shiro:hasPermission name="modules:century:banner:excel">
		...
		<script type="text/javascript">
			//基本路径
			var baseUrl = "<s:url value="/modules/century/banner"></s:url>/"
			
			//jqgrid的表格高度
			var height = 320;
			//jqgrid表格行头的中文
			var colNames = ['','标题','排序','备注','操作'];
			//jqgrid表格中每一行的信息模型
			var colModel = [
				...
				{name:'memo',index:'memo', width:60, editable: true},
			...
			function lookThis(data){
				...
				$("#formDialogBody").load('<s:url value="/modules/century/banner/edit"></s:url>/' + data);
				createDialog({
					dialogTitle : '详情',
					dialogFoot :  
			...
			$("#edit").click(function() {
				showEditDialog({
					...
					loadDialog: function(obj) {
						var name = 'bannerEdit';
						window.parent.openapp(obj.dialogUrl, name, '修改滚动图片');
                  }
				});
			});
			...
			function editThis(data){
				var url = '<s:url value="/modules/century/banner/edit"></s:url>/' + data;
				var name = 'bannerEdit';
				window.parent.openapp(url, name, '修改滚动图片');
			}
			//删除
			...
			$("#create").click(function(){
				var name = 'bannerCreate';				
				window.parent.openapp(getUrl("create"),name, '创建滚动图片');			
			});
